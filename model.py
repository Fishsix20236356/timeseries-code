...
class Chronos2Model(PreTrainedModel):
    ...
    def encode(
        ...
        # Two modes: (1) use pre-computed embeddings from MOMENT or (2) standard patching + embedding
        if inputs_embeds is not None:
            # Mode 1: Use pre-computed embeddings (from joint model with MOMENT)
            # inputs_embeds shape: (batch, num_history_patches + 1 + num_output_patches, d_model)
            # where +1 is for REG token (if use_reg_token=True)
            assert loc_scale is not None, "loc_scale must be provided when using inputs_embeds"

            batch_size = inputs_embeds.shape[0]
            total_patches = inputs_embeds.shape[1]

            # Calculate num_context_patches based on total_patches and num_output_patches
            # total_patches = num_context_patches + (1 if use_reg_token else 0) + num_output_patches
            if self.chronos_config.use_reg_token:
                num_context_patches = total_patches - num_output_patches - 1
            else:
                num_context_patches = total_patches - num_output_patches

            # Create attention mask (all ones since inputs are valid)
            attention_mask = torch.ones(batch_size, total_patches, dtype=self.dtype, device=self.device)

            # Create dummy patched_future_covariates_mask (all zeros since we don't use future covariates in joint model)
            patched_future_covariates_mask = torch.zeros(
                batch_size, num_output_patches, self.chronos_config.output_patch_size,
            ...
            # Mode 2: Standard patching and embedding (original Chronos-2 logic)
    ...
    def forward(
        ...
        inputs_embeds
            Pre-computed embeddings of shape (batch_size, total_sequence_length, d_model) to bypass patching and embedding.
            Used in joint training frameworks (e.g., MOMENT + Chronos-2) where external feature extractors provide embeddings.
            When provided, `loc_scale` must also be provided for loss computation. `context` parameter is ignored when
            `inputs_embeds` is provided.
        loc_scale
            Tuple of (loc, scale) tensors for instance normalization. Required when `inputs_embeds` is provided.
        ...
        """
        batch_size = inputs_embeds.shape[0] if inputs_embeds is not None else context.shape[0]
        ...
        expected_seq_len = num_context_patches + (1 if self.chronos_config.use_reg_token else 0) + num_output_patches
        assert hidden_states.shape == (batch_size, expected_seq_len, self.model_dim), \
            f"Expected shape ({batch_size}, {expected_seq_len}, {self.model_dim}), got {hidden_states.shape}"
